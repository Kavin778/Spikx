...
export default function socketHandler(io) {
    ...
    io.on('connection', (socket) =>{
        ...
        const isHost = async(roomId)=>{
            ...
            return room &&room.creatorId === room.id;
        }

        socket.on('host_play',async (data)=>{
            if(!(await isHost(data.roomId))){
                return console.log(`Permission denied ${socket.username}, you are not roomOwner bro, only he can control`);
            }
        ...
        socket.on('host_seek',async (data)=>{
            ...
            const currentState = roomStates[data.roomId] || {state:'paused',time:data.currenttime}
